...
const state = { board: new Map(), turn: "white", turnNumber: 1, lastMoved: null, status: "ongoing", mode: "idle", selectedHex: null, selectedHand: null, candidates: [], throwCandidates: [], imitation: null };
...
function highlightedHex(move) { return state.mode === "victim" ? move.victim : destination(move); }
...
function resetSelection() { state.mode = "idle"; state.selectedHex = null; state.selectedHand = null; state.candidates = []; state.throwCandidates = []; state.imitation = null; }
...
function selectBoard(hex) {
  ...
  state.selectedHex = hex; state.imitation = null; state.throwCandidates = candidates.filter(move => move.kind === "throw");
  if (direct.length) { state.mode = "destination"; state.candidates = direct; }
  else { state.mode = "victim"; state.candidates = state.throwCandidates; }
  render();
}
...
function imitationOptions() { return state.selectedHex && top(state.selectedHex)?.type === "mosquito" ? [...new Set(state.candidates.filter(move => move.kind === "mosquito").map(move => move.imitate).concat(state.throwCandidates.some(move => move.kind === "throw") ? ["pillbug"] : []))] : []; }
function chooseImitation(type) {
  if (!imitatable(state.selectedHex ?? []).includes(type) && !(type === "pillbug" && state.candidates.some(move => move.kind === "throw"))) return;
  state.imitation = type;
  if (state.mode === "destination") state.candidates = state.candidates.filter(move => move.kind !== "mosquito" || move.imitate === type);
  render();
}
function chooseAction(kind) { const candidates = kind === "Move" ? state.candidates.filter(move => move.kind === "movement" || (move.kind === "mosquito" && move.imitate === state.imitation)) : state.throwCandidates.length ? state.throwCandidates : state.candidates.filter(move => move.kind === "throw"); if (candidates.length) { state.mode = kind === "Move" ? "destination" : "victim"; state.candidates = candidates; render(); } }
...
function boardCells() { const result = new Set(state.board.keys()); for (const cell of state.board.keys()) for (const neighbor of neighbors(parse(cell))) result.add(key(...neighbor)); for (const move of state.candidates) result.add(key(...highlightedHex(move))); if (!result.size) result.add("0,0"); return [...result].map(parse); }
function layout() {
...
function draw() {
  ...
  const targetKeys = new Set(state.candidates.map(move => key(...highlightedHex(move))));
...
function renderMenu() { const menu = document.querySelector("#actionMenu"); menu.replaceChildren(); if (state.mode === "action") for (const option of actionOptions()) { const button = document.createElement("button"); button.type = "button"; button.textContent = option; button.addEventListener("click", () => chooseAction(option)); menu.append(button); } const selectedPiece = state.selectedHex ? top(state.selectedHex) : null; const canThrow = state.throwCandidates.length && (!selectedPiece || selectedPiece.type !== "mosquito" || state.imitation === "pillbug"); if (state.mode === "destination" && canThrow) { const button = document.createElement("button"); button.type = "button"; button.textContent = "Throw"; button.addEventListener("click", () => { state.mode = "victim"; state.candidates = state.throwCandidates; render(); }); menu.append(button); } if (["victim", "throw-destination"].includes(state.mode)) { const button = document.createElement("button"); button.type = "button"; button.textContent = "Cancel"; button.className = "cancel"; button.addEventListener("click", () => { const direct = legalMoves().filter(move => source(move) && same(source(move), state.selectedHex) && move.kind !== "throw"); state.mode = direct.length ? "destination" : "action"; state.candidates = direct.length ? direct : legalMoves().filter(move => source(move) && same(source(move), state.selectedHex)); render(); }); menu.append(button); } const imitation = document.querySelector("#imitationMenu"); if (imitation) { imitation.replaceChildren(); for (const type of imitationOptions()) { const button = document.createElement("button"); button.type = "button"; button.textContent = type; button.className = type === state.imitation ? "selected" : ""; button.addEventListener("click", () => chooseImitation(type)); imitation.append(button); } } }
